...
impl Mem for RiscvMem {
    ...
    fn map(root: &mut PT, vaddr: VAddr, paddr: PAddr, flags: PTEFlags) {
        ...
        v.set_paddr(paddr);
        v.set_flags(flags | PTEFlags::Valid);
    }
    ...
    fn v_to_p(root: &PT, vaddr: VAddr) -> Option<PAddr> {
        let vpn = PMode::vpn_indices(vaddr);
        let mut v = &root.entries[vpn[PMode::MAX_LEVELS - 1]];
